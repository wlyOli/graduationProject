<template>
	<div class="vegetable">
		...
		<el-form :model="model" 
		  ...
		  <el-form-item label="蔬菜标题">
			<el-col :span="12">
				<el-input v-model="model.title"></el-input>
			</el-col>
		  </el-form-item>
		  <el-form-item label="蔬菜价格">
			<el-col :span="6">
				<el-input v-model="model.price"></el-input>
			</el-col>
		  </el-form-item>
		  <el-form-item label="蔬菜类型">
	...
	export default{
		...
		methods:{
			...
			async save(){
				let res
				if(this.id){
					res = await this.$http.put(`rest/vegetables/${this.id}`,this.model) 
					
				}else{
					res = await this.$http.post('rest/vegetables',this.model)
				}
				//跳转到编辑详情页
				this.$router.push('/vegetables/list')
				//保存成功提示
				this.$message({
					message: '保存成功',
					type: 'success'
			...
			//获取品牌
			async fetch(){
				const res = await this.$http.get(`rest/vegetables/${this.id}`)
				this.model = res.data
			},
			//显示上级分类 
			async fetchType(){
