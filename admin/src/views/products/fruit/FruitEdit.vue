	...
	export default{
		...
		methods:{
			...
			async save(){
				let res
				if(this.id){
					res = await this.$http.put(`rest/fruits/${this.id}`,this.model) 
					
				}else{
					res = await this.$http.post('rest/fruits',this.model)
				}
				//跳转到编辑详情页
				this.$router.push('/fruits/list')
				//保存成功提示
				this.$message({
					message: '保存成功',
					type: 'success'
			...
			//获取品牌
			async fetch(){
				const res = await this.$http.get(`rest/fruits/${this.id}`)
				this.model = res.data
			},
			//显示上级分类 
			async fetchType(){
